...
def main():
    ...
    # Load model
    model_context = WandbModelContext(conf.wandb_prefix, conf.wandb_model_ref)

    method = model_huggingface_url_to_preprocessing_style(model_context.huggingface_dataset)

    # Load dataset
    try:
        ds = datasets.load_from_disk(str(Path(conf.save_datasets_path) / method.value / conf.split))

        assert isinstance(ds, datasets.Dataset), f"Expexted a Dataset, but found: {type(ds)=}"
    except FileNotFoundError:
        raise FileNotFoundError(f"\n\nYou should first run the dataset preprocessing script for {method.name=} and {conf.split=}")

    features_cache_folder = Path(conf.features_cache_folder) / conf.wandb_prefix / conf.wandb_model_ref / method.value / conf.split
